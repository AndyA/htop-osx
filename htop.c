...
printHelpFlag(  ) {
  ...
  printf
      ( "--sort-key COLUMN  Sort by this column (use --sort-key "
        "help for a column list)\n\n" );
...
#define addattrstr(a,s) attrset(a);addstr(s)
  ...
  if ( pl->detailedCPUTime ) {
    addattrstr( CRT_colors[CPU_NICE], "low" );
    addstr( "/" );
    ...
    addattrstr( CRT_colors[CPU_NICE], "low-priority" );
    addstr( "/" );
    addattrstr( CRT_colors[CPU_NORMAL], "normal" );
    addstr( "/" );
    addattrstr( CRT_colors[CPU_KERNEL], "kernel" );
  ...
  addstr( "/" );
  addattrstr( CRT_colors[MEMORY_BUFFERS], "buffers" );
  addstr( "/" );
  ...
  attrset( CRT_colors[DEFAULT_COLOR] );
  mvaddstr( 6, 0,
            "Type and layout of header meters are configurable "
            "in the setup screen." );
  if ( CRT_colorScheme == COLORSCHEME_MONOCHROME ) {
    mvaddstr( 7, 0,
              "In monochrome, meters are displayed through different "
              "chars, in order: |#*@$%&" );
  }
  mvaddstr( 8, 0,
            " Status: R: running; S: sleeping; T: traced/stopped; "
            "Z: zombie; D: disk sleep" );
  mvaddstr( 9, 0,
...
static char *CategoriesFunctions[10] =
    { "      ", "      ", "      ", "      ", "      ", "      ", "      ",
  "      ", "      ", "Done  "
};

static void
Setup_run( Settings * settings, int headerHeight ) {
  ScreenManager *scr =
      ScreenManager_new( 0, headerHeight, 0, -1, HORIZONTAL, true );
  CategoriesPanel *panelCategories = CategoriesPanel_new( settings, scr );
  ScreenManager_add( scr, ( Panel * ) panelCategories,
                     FunctionBar_new( CategoriesFunctions, NULL, NULL ),
                     16 );
...
static bool
changePriority( Panel * panel, int delta ) {
  bool ok = true;
...
pickWithEnter( Panel * panel, int ch ) {
  if ( ch == 13 )
    return BREAK_LOOP;
  return IGNORED;
}
...
static Object *
pickFromVector( Panel * panel, Panel * list, int x, int y,
                char **keyLabels, FunctionBar * prevBar ) {
  ...
  ScreenManager_add( scr, list,
                     FunctionBar_new( keyLabels, fuKeys, fuEvents ),
                     x - 1 );
...
static void
addUserToVector( int key, void *userCast, void *panelCast ) {
...
static void
setUserOnly( const char *userName, bool * userOnly, uid_t * userId ) {
...
static inline void
setSortKey( ProcessList * pl, ProcessField sortKey, Panel * panel,
            Settings * settings ) {
  pl->sortKey = sortKey;
  pl->direction = 1;
  pl->treeView = false;
...
main( int argc, char **argv ) {
  ...
  panel =
      Panel_new( 0, headerHeight, COLS, LINES - headerHeight - 2,
                 PROCESS_CLASS, false, NULL );
  if ( sortKey > 0 ) {
    pl->sortKey = sortKey;
    pl->treeView = false;
    pl->direction = 1;
  ...
  FunctionBar *searchBar =
      FunctionBar_new( searchFunctions, searchKeys, searchEvents );

  char *defaultFunctions[] =
      { "Help  ", "Setup ", "Search", "Invert", "Tree  ",
    "SortBy", "Nice -", "Nice +", "Kill  ", "Quit  ", NULL
  };
  FunctionBar *defaultBar =
      FunctionBar_new( defaultFunctions, NULL, NULL );
  ...
  while ( !quit ) {
    ...
    newTime =
        ( ( double ) tv.tv_sec * 10 ) + ( ( double ) tv.tv_usec / 100000 );
    ...
    if ( doRefresh ) {
      ...
      for ( int i = 0; i < size; i++ ) {
        ...
        if ( !userOnly || ( p->st_uid == userId ) ) {
          ...
          if ( ( !follow && index == currPos )
               || ( follow && p->pid == currPid ) ) {
    ...
    if ( incSearchMode ) {
      ...
      if ( found )
        ...
        FunctionBar_drawAttr( searchBar, incSearchBuffer,
                              CRT_colors[FAILED_SEARCH] );

      continue;
    }
    if ( isdigit( ( char ) ch ) ) {
      unsigned int pid = ch - 48 + acc;
      for ( int i = 0; i < ProcessList_size( pl )
            && ( ( Process * ) Panel_getSelected( panel ) )->pid != pid;
            i++ )
        Panel_setSelected( panel, i );
      acc = pid * 10;
      if ( acc > 100000 )
    ...
    if ( ch == KEY_MOUSE ) {
      ...
      if ( ok == OK ) {
        if ( mevent.y == panel->y ) {
          ...
          Panel_setSelected( panel,
                             mevent.y - panel->y + panel->scrollV - 1 );
          doRefresh = false;
    ...
    case 's':
      {
        TraceScreen *ts =
            TraceScreen_new( ( Process * ) Panel_getSelected( panel ) );
        TraceScreen_run( ts );
    ...
    case 'l':
      {
        OpenFilesScreen *ts = OpenFilesScreen_new( ( Process * )
                                                   Panel_getSelected
                                                   ( panel ) );
    ...
    case 'u':
      {
        ...
        ListItem *picked =
            ( ListItem * ) pickFromVector( panel, usersPanel, 20,
                                           headerHeight, fuFunctions,
                                           defaultBar );
        if ( picked ) {
          if ( picked == allUsers ) {
            userOnly = false;
    ...
    case 'k':
      {
        ...
        Signal *signal = ( Signal * ) pickFromVector( panel, killPanel, 15,
                                                      headerHeight,
                                                      fuFunctions,
                                                      defaultBar );
        if ( signal ) {
    ...
    case 'a':
      {
        ...
        Panel *affinityPanel =
            AffinityPanel_new( pl->processorCount, curr );

        char *fuFunctions[2] = { "Set    ", "Cancel " };
        void *set = pickFromVector( panel, affinityPanel, 15, headerHeight,
                                    fuFunctions, defaultBar );
        if ( set ) {
    ...
    case KEY_F( 6 ):
      {
        ...
        for ( int i = 0; fields[i]; i++ ) {
          ...
          Panel_add( sortPanel,
                     ( Object * ) ListItem_new( name, fields[i] ) );
          if ( fields[i] == pl->sortKey )
            Panel_setSelected( sortPanel, i );
          free( name );
        }
        ListItem *field =
            ( ListItem * ) pickFromVector( panel, sortPanel, 15,
                                           headerHeight, fuFunctions,
                                           defaultBar );
        if ( field ) {
          settings->changed = true;
