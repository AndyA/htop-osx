...
static boolean_t
libtop_p_proc_command( libtop_pinfo_t * a_pinfo,
                       struct kinfo_proc *a_kinfo );
static void
...
libtop_p_oinfo_insert( int a_obj_id, int a_share_type,
                       int a_resident_page_count, int a_ref_count,
                       int a_size, libtop_pinfo_t * a_pinfo );
static void
...
libtop_fini( void ) {
  ...
  for ( ;
        dch_remove_iterate( &libtop_uhash, NULL, ( void ** ) &user, NULL )
        == FALSE; ) {
    free( user );
  }
  ...
  if ( a_fw ) {
    for ( fw_seen = FALSE, addr = GLOBAL_SHARED_TEXT_SEGMENT;
          addr < ( GLOBAL_SHARED_DATA_SEGMENT
                   + SHARED_DATA_REGION_SIZE ); addr += size ) {
      /*
      ...
      if ( addr < GLOBAL_SHARED_DATA_SEGMENT ) {
        if ( sinfo.share_mode == SM_SHARED || sinfo.share_mode == SM_COW ) {
...
libtop_p_vm_sample( void ) {
   ...
   */
  tsamp.rshrd = 0;
  for ( i = 0, ocount = dch_count( &libtop_oinfo_hash ); i < ocount; i++ ) {
    dch_get_iterate( &libtop_oinfo_hash, NULL, ( void ** ) &oinfo );
    if ( oinfo->share_type == SM_SHARED || oinfo->share_type == SM_COW ) {
...
libtop_p_task_update( task_t a_task, boolean_t a_reg ) {
  ...
  if ( ( a_reg && pinfo->preg != LIBTOP_PREG_off )
     ...
     */
    for ( address = 0, pinfo->split = FALSE;; address += size ) {
      /* Get memory region. */
      count = VM_REGION_TOP_INFO_COUNT;
      if ( vm_region( a_task, &address, &size,
                      VM_REGION_TOP_INFO, ( vm_region_info_t ) & info,
      ...
      if ( address >= GLOBAL_SHARED_TEXT_SEGMENT
        ...
        tsamp.fw_private += info.private_pages_resident * tsamp.pagesize;
      ...
      case SM_COW:{
          if ( info.ref_count == 1 ) {
            ...
            pinfo->psamp.rprvt
                += info.shared_pages_resident * tsamp.pagesize;
          }
          else {
            /*
            ...
            if ( pinfo->psamp.pid == 0 ) {
               ...
               */
              pinfo->psamp.rprvt
                  += info.private_pages_resident * tsamp.pagesize;
              pinfo->psamp.vprvt
                  += info.private_pages_resident * tsamp.pagesize;
              break;
            }
            ...
            if ( oinfo->share_type == SM_SHARED
                 && oinfo->ref_count == oinfo->proc_ref_count ) {
              /*
              ...
              oinfo->rb_aliased
                  += oinfo->resident_page_count * tsamp.pagesize;
              aliased += oinfo->resident_page_count * tsamp.pagesize;
      ...
      case SM_SHARED:{
          ...
          if ( oinfo->share_type != SM_PRIVATE_ALIASED ) {
            oinfo->rb_rshrd += oinfo->resident_page_count * tsamp.pagesize;
            pinfo->psamp.rshrd
                += oinfo->resident_page_count * tsamp.pagesize;
          }
  ...
  for ( i = 0; i < table_size; i++ ) {
    ...
    if ( thread_info( thread_table[i], THREAD_BASIC_INFO,
      ...
      if ( ( thi->flags & TH_FLAGS_IDLE ) == 0 ) {
        ...
        timeradd( &pinfo->psamp.user_time, &tv, &pinfo->psamp.user_time );
  ...
  if ( ( error =
         ...
         KERN_SUCCESS ) ) {
    libtop_print( libtop_user_data, "Error in vm_deallocate(): %s",
                  mach_error_string( error ) );
    retval = TRUE;
    goto RETURN;
  }
  ...
  if ( mach_port_names( a_task, &names, &ncnt, &types, &tcnt )
    ...
    if ( ( error =
           vm_deallocate( mach_task_self(  ), ( vm_offset_t ) names,
                          ncnt * sizeof( mach_port_array_t ) ) ) !=
         KERN_SUCCESS
         || ( error =
              vm_deallocate( mach_task_self(  ), ( vm_offset_t ) types,
                             tcnt * sizeof( mach_port_array_t ) ) )
         != KERN_SUCCESS ) {
      libtop_print( libtop_user_data,
                    "Error in vm_deallocate(): %s",
                    mach_error_string( error ) );
  ...
  if ( task_info( a_task, TASK_EVENTS_INFO,
                  ( task_info_t ) & pinfo->psamp.events,
                  &count ) != KERN_SUCCESS ) {
    /* Error. */
    state = LIBTOP_STATE_ZOMBIE;
    retval = FALSE;
...
libtop_p_oinfo_fini( void ) {
  ...
  for ( oinfo = ql_last( &libtop_oinfo_list, link );
        oinfo != NULL; oinfo = ql_last( &libtop_oinfo_list, link ) ) {
...
libtop_p_oinfo_insert( int a_obj_id, int a_share_type,
  ...
  if ( dch_search
       ( &libtop_oinfo_hash, ( void * ) a_obj_id, ( void ** ) &oinfo )
       == FALSE ) {
    /* Use existing record. */
